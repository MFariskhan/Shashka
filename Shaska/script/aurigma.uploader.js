...
var axVERSION = '8.5.67.0', jVERSION = '8.5.67.0';
var uploaderClassID = '89901286-6A4E-4CE6-9771-194A6B176587', uploaderProgID = 'Aurigma.Uploader.8567.1';
var thumbnailClassID = '7931A49C-F5DF-454E-8EB0-78606F479CC8', thumbnailProgID = 'Aurigma.Thumbnail.8567.1';
...
browser = AU.browser = new (function () {
    ...
    this.isEdge = (a.indexOf("edge") > -1);
    this.isOpera = (/opera|opr/g.test(a));
    this.isKonq = (a.indexOf("konqueror") > -1);
    this.isChrome = (a.indexOf("chrome") > -1) && !this.isEdge && !this.isOpera;
    this.isSafari = (a.indexOf("safari") > -1) && !this.isChrome && !this.isEdge;
    ...
    this.isBeforeIE6XPSP2 = this.isIE && !this.isIE6XPSP2 && !this.isIE7 && !this.isIE8 && !this.isIE9 && !this.isIE11;
...
AU.fileAccessChecker = function (uploader) {
    if (!(this instanceof AU.fileAccessChecker)) {
        return new AU.fileAccessChecker(uploader);
    }
    ...
    var checkFileAccess = function() {
        var el = document.getElementById(uploader.id() + "-overlay");

        if (!document.getElementById(uploader.id()).isFileAccessGranted()) 
    ...
    var f = function () {
        if (browser.isChrome) {
            var w = window.document.getElementById('internal-aurigma-warning');
            if (!!w) {
                w.style.display = "none";
            }
        }

        if (browser.isMac && browser.isSafari)
...
AU.fileAccessChecker.prototype.__class = true;

AU.fileAccessChecker.prototype.onJavaAfterCloseTagRender = function (uploader, args) {
    if (browser.isMac && browser.isSafari) {
        var html = [];
        var message = uploader.messages().noFileSystemAccessInSafari().replace('[name]', location.hostname);

        html.push('</div>');
        html.push('<div id="' + uploader.id() + '-overlay" style="visibility:hidden;position:absolute;left:0px;top:0px;width:100%;height:100%;text-align:left;z-index:1000;font-family: Tahoma, Verdana, Arial, Calibri, Sans-Serif; font-size: 13px;">');
        html.push('<div style="width:410px;margin:200px auto;background-color:#f7f7f7;border:1px solid #aaa;padding:15px;text-align:left;box-shadow: 0 0 10px rgba(0,0,0,0.5)">');
        html.push(message);
        html.push('<p style="text-align:center"><button style="width:60px;height:21px;font-size:13px;!important" type="button" onclick="');
        html.push('var code = document.getElementById(\'' + uploader.id() + '-checker\').innerHTML;');
        html.push('document.getElementById(\'' + uploader.id() + '-checker\').innerHTML = \'\';');
        html.push('document.getElementById(\'' + uploader.id() + '-checker\').innerHTML = code;');
        html.push('document.getElementById(\'' + uploader.id() + '-overlay\').style.visibility = \'hidden\';');
        html.push('">Retry</button></p>');
        html.push('</div>');
        html.push('</div>');
...
AU.fileAccessChecker.prototype.onJavaBeforeOpenTagRender = function (uploader, args) {
    if (browser.isMac && browser.isSafari) {
        args.resultHtml = "<div id='" + uploader.id() + "-checker' style='height:100%'>";
    }
};
AU.debug = function () {
    ///	<summary>
    ...
    getHtml: function () {
        ...
        var createEventHandler = function (event, eventName) {
            var h = function () {
            	...
            	if (eventName == 'initComplete' && (self instanceof AU.uploader) && self.type() == 'java') {
                    try {
                        // Check applet is active and can call its methods
                        self.getElement().getVersion();
                        self.getElement().getConverterCount();
                    } catch (err) {
                        showError('[js_info] Applet is not ready. Waiting...');
                        setTimeout(h, 200);
                        return;
                    }
                }
...
AU.activeXControl = function (uploader) {
    this._uploader = uploader;
    this.codeBase('Scripts/Uploader8.cab');
    this.codeBase64('Scripts/Uploader8_x64.cab');
};

AU.activeXControl.prototype = {
    __class: true,

    isActiveXInstalled: function () {
        try {
            var a = new ActiveXObject(this.progId());
            return true;
        } catch (err) {
            // Not installed if error
        }
        return false;
    },
    ...
    isActiveXSupported: function () {
        return (typeof window.ActiveXObject !== undefinedStr) || (AU.browser.isIE11 && ("ActiveXObject" in window)); // True if window.ActiveXObject exists.
    },
    ...
    version: function (value) {
        ///	<summary>
        ///		Gets or sets minimum required version of the ActiveX control (Uploader8.cab file).
        ///	</summary>
        ///	<param name="value" type="String">
...
AU.file.reinit = function (element, obj) {
    ...
    obj.asBase64 = function () {
        var result = element.getUploadFileAsBase64(this._index);
...
AU.javaControl = function (uploader) {
    this._uploader = uploader;
    this.codeBase('Scripts/Uploader8.jar');
};
...
AU.javaControl.prototype = {
    ...
    version: function (value) {
        ///	<summary>
        ///		Gets or sets minimum required version of the Java applet (Uploader8.jar file).
        ///	</summary>
        ///	<param name="value" type="String">
        ///     Version string in the x.x.x.x format
        ///	</param>
...
AU.messages = function () {
    ...
    this._noFileSystemAccessInSafari = '<p>The browser does not allow Upload Suite to access the local file system. To enable the access perform the following steps:</p>' +
        '<ul>' + 
        '<li style="margin-top:10px">Go to <strong>Safari</strong> &rarr; <strong>Preferences</strong>, choose the <strong>Security</strong> tab, and click <strong>Plug-in Settings...</strong> near <strong>Internet Plug-ins</strong>.</li>' +
        '<li style="margin-top:10px">Select <strong>Java</strong> in the left column.</li>'+
        '<li style="margin-top:10px">Hold the <strong>Option/Alt</strong> button.</li>'+
        '<li style="margin-top:10px">Click on the dropdown box next to the <strong>[name]</strong> website. Choose <strong>Run in Unsafe Mode</strong> or uncheck <strong>Run in Safe Mode</strong>.</li>' + 
        '<li style="margin-top:10px">If a security dialog appears, click the <strong>Trust</strong>  button.</li>'+
        '</ul>';
}
...
AU.messages.prototype._prop = function (name, value, action) {
    if (action) {
        // set passed value
        this['_' + name] = value;
    } else {
...
AU.messages.prototype.noFileSystemAccessInSafari = function (value) {
    return this._prop('noFileSystemAccessInSafari', arguments[0], arguments.length);
};
...
AU.paneItem.prototype = {
    ...
    _simpleProperties: [
        { name: "checkedHoverBorderColor", getter: 1, setter: 1 },
        { name: "checkedHoverColor", getter: 1, setter: 1 },
        ...
        { name: "checkedSelectionBorderColor", getter: 1, setter: 1 },
        { name: "checkedSelectionColor", getter: 1, setter: 1 },
        { name: "checkedSelectionHoverBorderColor", getter: 1, setter: 1 },
        { name: "checkedSelectionHoverColor", getter: 1, setter: 1 },
        ...
        { name: "inactiveCheckedHoverBorderColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedHoverColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedItemBorderColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedItemColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedSelectionBorderColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedSelectionColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedSelectionHoverBorderColor", getter: 1, setter: 1 },
        { name: "inactiveCheckedSelectionHoverColor", getter: 1, setter: 1 },
    ...
    checkedHoverBorderColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    checkedHoverColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    ...
    checkedSelectionBorderColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    checkedSelectionColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    checkedSelectionHoverBorderColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    checkedSelectionHoverColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    ...
    inactiveCheckedHoverBorderColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    inactiveCheckedHoverColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    ...
    inactiveCheckedSelectionBorderColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    inactiveCheckedSelectionColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    inactiveCheckedSelectionHoverBorderColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
    inactiveCheckedSelectionHoverColor: function (value) {
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
    },
...
AU.statusPane.prototype = {
	...
	_simpleProperties: [
		...
		{ name: "addButtonText", getter: 1, setter: 1 }, // private label only
		{ name: "addAllButtonText", getter: 1, setter: 1 }, // private label only
		{ name: "removeButtonText", getter: 1, setter: 1 }, // private label only
		{ name: "removeAllButtonText", getter: 1, setter: 1 }, // private label only
		{ name: "addButtonImageFormat", getter: 1, setter: 1 }, // private label only
		{ name: "addAllButtonImageFormat", getter: 1, setter: 1 }, // private label only
		{ name: "removeButtonImageFormat", getter: 1, setter: 1 }, // private label only
		{ name: "removeAllButtonImageFormat", getter: 1, setter: 1} // private label only
	],
	getParams: getParams,
	...
	addButtonText: function (value) {
		/// <summary>
		///     Get or set "Add" button text
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	addAllButtonText: function (value) {
		/// <summary>
		///     Get or set "Add all" button text
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	removeButtonText: function (value) {
		/// <summary>
		///     Get or set "Remove" button text
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	removeAllButtonText: function (value) {
		/// <summary>
		///     Get or set "Remove all" button text
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	addButtonImageFormat: function (value) {
		/// <summary>
		///     Get or set "Add" button image format
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	addAllButtonImageFormat: function (value) {
		/// <summary>
		///     Get or set "Add all" button image format
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	removeButtonImageFormat: function (value) {
		/// <summary>
		///     Get or set "Remove" button image format
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	},
	removeAllButtonImageFormat: function (value) {
		/// <summary>
		///     Get or set "Remove all" button image format
		/// </summary>
		///	<param name="value" type="String" />
		///	<returns type="String" />
	}
...
AU.uploadPane.prototype = {
    ...
    _simpleProperties: [
        ...
        { name: "dropFilesHereMacImageFormat", getter: 1, setter: 1 },
        { name: "dropFilesHereMacText", getter: 1, setter: 1 },
    ...
    dropFilesHereMacImageFormat: function (value) {
        ///	<summary>
        ///		Get or set image format for "Drop file here" label on Mac.
        ///	</summary>
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
        /* will be created while initialization */
    },
    dropFilesHereMacText: function (value) {
        ///	<summary>
        ///		Get or set "Drop files here" label text on Mac.
        ///	</summary>
        ///	<param name="value" type="String" />
        ///	<returns type="String" />
        /* will be created while initialization */
    ...
    // addAllFiles is not available in standard Aurigma Upload Suite version due the security reasons
    addFiles: function () {
        ///	<summary>
    ...
    // addFileByPath is not available in standard Aurigma Upload Suite version due the security reasons
    addFolders: function () {
        ///	<summary>
    ...
    // addFolderByPath is not available in standard Aurigma Upload Suite version due the security reasons.
    // addSelectedFiles is not available in standard Aurigma Upload Suite version due the security reasons.
...
AU.uploadSettings.prototype = {
    ...
    autoRecoveryTimeout: function (value) {
        ///	<summary>
        ///		Get or set interval in which Aurigma Upload Suite should try to resume the upload if it was interrupted.
        ///	</summary>
        ///	<param name="value" type="Number">
        ///	</param>
        ///	<returns type="Number" />
    ...
    charset: function (value) {
        ///	<summary>
        ///		Get or set charset used to encode the text data submitted by Aurigma Upload Suite.
        ///	</summary>
        ///	<param name="value" type="String">
        ///	</param>
        ///	<returns type="String" />
    ...
    enableHeadRequest: function (value) {
        ///	<summary>
	...
	maxConnectionCount: function (value) {
        ///	<summary>
        ///		Get or set max number of connections.
        ///	</summary>
        ///	<param name="count" type="Number" />
        ///	<returns type="Number" />
    },
...
AU.uploader = function (obj) {
    ...
    var u = new AU.uploader.fn.init(obj);
    $au.fileAccessChecker(u);
...
extend(AU.uploader.fn, {
    _simpleProperties: [
        ...
        { name: "openFileDialog", getter: 1, setter: 1 },
    ...
    _methods: [
		...
		//for Thumbnail control
		{ name: "getUploadFileName", controlMethodName: "getUploadFileName"},   
		{ name: "getUploadFileAngle", controlMethodName: "getUploadFileAngle"}, 
		{ name: "setUploadFileThumbnailId", controlMethodName: "setUploadFileThumbnailId"}
    ],
    init: function (initObj) {
        ...
        this.javaControl({
        	className: 'com.aurigma.uploader.Uploader.class',
            version: jVERSION
        });
    ...
    openFileDialog: function (value) {
        ///	<summary>
        ///		Get or set add files dialog type on Mac. 
        ///	</summary>
        ///	<param name="value" type="String">
        ///		One of the values "Standard", "MacOSNative" or equivalent number value
        ///	</param>
        ///	<returns type="String" />
    },
	...
	getUploadFileAngle: function () {
	},
	setUploadFileThumbnailId: function () {
	}	
}, true);
...
extend(AU.thumbnail.fn, {
    ...
    init: function (initObj) {
        ...
        this._javaControl = new AU.javaControl(this);
        this.javaControl({
            className: 'com.aurigma.uploader.Thumbnail.class',
            version: jVERSION
        });
	...
	bind: function () { //Bind with Uploader
		if (this.type() == 'java')  {
			var parentId = this.parentControlName();
			var guid = this.guid();
			if (parentId && guid) {
				var parent = $au.uploader(parentId);
				this.angle(parent.getUploadFileAngle(guid));
				this.fileName(parent.getUploadFileName(guid));			
				parent.setUploadFileThumbnailId(guid, this.id());
			} else {
				this.fileName("");
				this.angle(0);
			}
...
var javaRenderer = function (uploader, options) {
	...
	var getHtml = function () {
		...
		if (browser.isChrome || browser.isEdge) {
			html.push("<style>");
			html.push("#internal-aurigma-warning { color: #333333; padding: 5px; margin: 10px; text-align: left; border: solid 1px; border-color: #999999;}");
			html.push("</style>");
			html.push("<div id='internal-aurigma-warning' id='java-applet-not-supported-info'>");				
		
			if(browser.isChrome){
                   		html.push("Chrome no longer supports Java applets.")
			}else if(browser.isEdge){
                    		html.push("Edge does not support Java applets and ActiveX controls.")
			}

		    	html.push(" Please switch to Firefox or Internet Explorer to proceed.");
		    	html.push("</div>");
		}
		...
		if (useObjectTag) {
			...
			if (!browser.isIE64) {
				...
				attributes.codebase = window.location.protocol + "//java.sun.com/update/1.7.0/jinstall-7u45-windows-i586.cab#Version=" + javaVersion;
			}
...
if(!window.$au && typeof $au != "undefined")
      window.$au = $au;

if (window.$au) {
	window.$au.uploader = $au.uploader;	
}
})(window);
